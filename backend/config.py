...
class Settings:
    ...
    DEBUG_OTP: bool = os.getenv("DEBUG_OTP", "False").lower() in ("true", "1", "yes")
    ...
    VAPID_CLAIMS_EMAIL: str = os.getenv("VAPID_CLAIMS_EMAIL", "admin@smartparkride.local")
    ...
    def validate(self) -> None:
        ...
        # JWT_SECRET: must be set in production; fallback to default in dev
        if not self.JWT_SECRET:
            if is_production:
                raise RuntimeError(
                    "JWT_SECRET must be set in production environment."
                )
            self.JWT_SECRET = "smart-park-and-ride-default-jwt-secret-v1"
            warnings.warn(
                "JWT_SECRET is not set. Using insecure default for development. "
        ...
        if is_production:
            ...
            for origin in self.CORS_ALLOWED_ORIGINS.split(","):
                origin = origin.strip()
                if origin and ("ngrok" in origin.lower() or "localhost" in origin.lower()):
                    warnings.warn(
                        f"Potentially unsafe origin in production CORS config: {origin}"
                    )
        else:
            if not self.ADMIN_PASSWORD.startswith("$2"):
                warnings.warn(
